...
class UCF_CC_50(Base):
    def __init__(self, root_path, crop_size, downsample_ratio=8, method='train'):
        super().__init__(root_path, crop_size, downsample_ratio)
        self.method = method
        self.im_list = sorted(glob(os.path.join(self.root_path, '*.jpg')))
        print('number of images: {}'.format(len(self.im_list)))

        if method not in ['train', 'val']:
            raise Exception("Invalid method: Choose either 'train' or 'val'")
    ...
    def __getitem__(self, item):
        ...
        # Load the ground truth keypoints from the .mat file
    ...
    def train_transform(self, img, keypoints):
        ...
        assert st_size >= self.c_size, "Image size is smaller than the crop size"
        assert len(keypoints) >= 0, "No keypoints found"
